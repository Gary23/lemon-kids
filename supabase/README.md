...
| `sql/20260906_family_video_library.sql` | 家庭动画 App 的云盘连接、分类、剧集、视频、播放记录和同步日志表及初版仅家长 RLS | 已有 `users`、`families`；上线家庭动画前先执行。令牌与云盘密码不得入库。 |
| `sql/20260906_family_video_all_family_access.sql` | 将家庭动画的 RLS 从仅家长改为家庭内所有登录用户可访问 | 已执行 `20260906_family_video_library.sql`；当前家庭动画 App 不区分家长/孩子权限，必须继续执行。完整开发与验收见 `../family-video-app/docs/DEVELOPMENT.md`。 |
