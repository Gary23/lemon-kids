...
认字端请求 `archive_recognized_character` 时，评测函数会调用数据库 RPC；它先确保字已幂等写入 `known_characters`，再将该已认识字及（系统来源时）原认字任务的全部资产标记为 `delete_pending`，并立即清空字、词、句的音频 URL。Storage 删除不在请求链路中执行，因此临时删除失败不会撤销存库。

为本函数新增一个每 5 分钟运行一次的事件触发器，事件内容如下；也可由有 `scf:InvokeFunction` 权限的管理端调用：
...
测试不访问腾讯云、Supabase 或真实音频，适合每次打包前执行。真实环境的归档端到端验收必须使用测试孩子和临时教学数据：先完成一条含字、词、句的认字任务，确认生成对象均为 `ready`；在认字端点击“存入字库”两次；确认 `known_characters` 只有一条、关联资产先变为 `delete_pending`，最后由清理任务自动物理删除关联资产及 `recognized_characters`。手工/导入的已认识字也按同样步骤单独验收。不要对生产中的学习记录执行此测试。

要验证“部分对象删除失败后重试”，在测试环境先把清理函数的 `SUPABASE_URL` 临时改为不可访问地址，手动触发一次 `cleanup`，确认 `known_characters` 记录仍存在、资产回到 `delete_pending`；立即恢复原变量并再次触发 `cleanup`，确认资产最终为 `deleted`。此操作会让该函数在变量恢复前无法清理，禁止在生产环境执行。
