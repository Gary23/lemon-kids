...
家长端新建时必须从当前孩子已绑定的监控 Pad 中明确选择目标；保存前再次校验该设备 ID 仍在列表中。这样同一孩子绑定多台 Pad 时，不会把闹钟投递到不确定的设备。
...
监控端恢复已保存会话时先初始化 Auth，再初始化 PostgREST，并在后台 Worker 对账前补做一次会话恢复；否则 PostgREST 可能以匿名身份查询并得到空列表。服务端 `TIMESTAMPTZ` 的 `+00:00` 结尾在登记前规范化为 `Z`，兼容旧设备的时间解析器。
...
家长可从本家庭曲库选择或上传 MP3/OGG 背景音乐；上传文件限制为 5 MB、30～90 秒。家庭文件存于私有 `alarm-family-background-music` bucket 的 `custom/{family_id}/{music_id}/v1/background.{ogg|mp3}`，闹钟只保存 `background_music_id`。Pad 在联网同步期获取短时下载地址、校验 MIME/大小/SHA-256 并写入应用私有缓存；到点只播放已验证的本地文件，下载失败或文件损坏即回退 `gentle_bell_v1`。

背景音乐与本地 TTS 同时播放、不暂停也不压低；背景固定响度为原设定的 3 倍。音频文件、签名 URL、请求头和会话令牌均不得写入闹钟数据、日志或界面。
...
一期默认：在生效日期范围内每天响铃、振动、锁屏全屏页、手动关闭，单次最长响 5 分钟。家长端配置的 `requires_confirmation` 已预留，二期可扩展为答题、朗读、拍照或家长远程确认；扩展必须由 `AlarmActivity` 的完成态驱动，不能让通知划掉即视为完成。
